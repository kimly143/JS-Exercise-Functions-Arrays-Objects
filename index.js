...
function getCarInfoById(inventory, id) {
  for(let i=0; i<inventory.length; i++){
    ...
    if (record.id === id) {
      const carMakebyId = record.car_make;
      const carModelbyId = record.car_model;
      return `This is a ${carMakebyId} ${carModelbyId}`;
    }
  }
}
...
function getModelYears(inventory) {
  const carYears = [];
  for (let i=0 ; i< inventory.length; i++){
    const iCarYear = inventory[i].car_year;
    carYears.push(iCarYear);
  }
  return carYears;
}
